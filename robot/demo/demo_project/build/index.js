...
import { SmartLed, LED_WS2812, LED_WS2812B_2020 } from "smartled";
...
function mapADC(rangeFrom, rangeTo, num) {
    let result = (num / (rangeFrom / rangeTo));
    return Math.round(result);
}
function configureLedStripExternal() {
    adc.configure(JOY_PM12_X);
    adc.configure(JOY_PM12_Y);
    let ledStrip = new SmartLed(SMART_LED_PIN_EX, SMART_LED_PIN_EX_COUNT, LED_WS2812B_2020);
    setInterval(() => {
        ...
        for (let i = 0; i <= lenght; i++) { // nastavíme barvu na všechny ledky v rozsahu délky
